...
class LoginViewController: UIViewController {
    ...
    @IBAction func loginTapped(_ sender: Any) {
        ...
        Auth.auth().signIn(withEmail: email, password: password) { (result, error) in
            ...
            else{//성공 하면 성공 문자열로, 클로저 안들은 self 키워드
                self.pushView(controller: Constants.Storyboard.clvc)
            }
        }
    }
    //구글 로그인 버튼
    @IBAction func googleLogin(_ sender: Any) {
        ...
        if let user = user {
            let name = user.displayName ?? "김길동"
            let password = user.uid
            let email = user.email!
            self.saveFirebase(name, email, password)
            
            //viewDidLoad 함수에서도 여부 상관 없이 그냥 이동합니다...
    ...
    func loginFirebase(_ name: String, _ email: String, _ password: String) {
        Auth.auth().signIn(withEmail: email, password: password) { (result, error) in
            if error != nil {
                ...
                let homeViewController = self.storyboard?.instantiateViewController(identifier: Constants.Storyboard.clvc) as? CompleteLoginViewController
        ...
        UserApi.shared.me() {(user, error) in
            if let error = error {
                ...
                if let user = user {
                    ...
                    // https://developers.kakao.com/docs/latest/ko/user-mgmt/ios
                    // scopes에 내용을 받아온다, 안 받아온다고 지우면 에러 생김
                    var scopes = [String]()
    ...
    func saveFirebase(_ name: String, _ email:String, _ password: String){
       ...
       db.child("/users/\(name)").setValue(["id":email,"password":password]) //유저밑에 만들고 싶어서 이렇게 했는데 잘 되려나

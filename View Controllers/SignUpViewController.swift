...
class SignUpViewController: UIViewController {
    ...
    @IBAction func signUpTapped(_ sender: Any) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in   guard (result?.user) != nil else { return }
            ...
            if error == nil {
                ...
                //홈으로 다시 돌아가기
                alert.addAction(UIAlertAction(title:"확인", style:UIAlertAction.Style.default, handler:{Action in self.goHome()}))
    ...
    func goHome(){
        //as?는 캐스팅 실패시 nil 반환
        let homeViewController = storyboard?.instantiateViewController(identifier: Constants.Storyboard.clvc) as? CompleteLoginViewController
